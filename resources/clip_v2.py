...
def classify_image_with_clip(image_path, top_k=10, threshold=0.25):
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python clip_multilabel.py /path/to/image.jpg [top_k=10] [threshold=0.25]")
        sys.exit(1)
    ...
    # Run classification
    results = classify_image_with_clip(image_path, top_k=top_k, threshold=threshold)

    print(f"Image: {image_path}")
    print(f"Top {top_k} Tags (score > {threshold}):")
